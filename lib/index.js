    ...
    .catch(err => {
      if (err.response) {
        ...
        if (err.response.status === 401) {
          throw new Error(`ION JSON-RPC: Connection Rejected: 401 Unnauthorized`)
        }
        if (err.response.status === 403) {
          throw new Error(`ION JSON-RPC: Connection Rejected: 403 Forbidden`)
        }
        let error = new Error(`ION JSON-RPC: ${request.method} fail: ${err.response.data.error.code} ${err.response.data.error.message}`)
...
RpcClient.callspec = {
  ...
  // == Tokens ==
  configureManagementToken: 'str str int str str bool',
  configureToken: 'str str int str str bool',
  createTokenAuthorities: 'str str',
  dropTokenAuthorities: 'str str int',
  getSubgroupId: 'str str',
  getTokenBalance: 'str',
  getTokenTransaction: 'str str',
  listTokenAuthorities: 'str',
  listTokensSinceBlock: 'str str int bool',
  listTokenTransactions: 'str int int bool',
  meltToken: 'str int',
